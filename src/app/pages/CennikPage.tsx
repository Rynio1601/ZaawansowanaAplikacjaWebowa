...
export function CennikPage() {
  ...
  return (
    <div style={{ paddingTop: 100 }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
            {plans.map(({ name, price, period, color, desc, features, cta, recommended }) => (
              <div key={name} className={`relative p-6 rounded-2xl flex flex-col ${recommended ? 'ring-2' : ''}`}
                style={{ background: '#0A0F1A', border: `1px solid ${recommended ? color : 'rgba(255,255,255,0.06)'}`, boxShadow: recommended ? `0 0 40px ${color}20` : 'none', ringColor: color }}>
                {recommended && (
      ...
      {/* FAQ mini */}
      <section className="py-16 text-center">
        <p style={{ color: '#475569' }}>Masz pytania? <Link to="/demo" style={{ color: '#60A5FA' }}>Porozmawiaj z nami</Link> lub <Link to="/demo" style={{ color: '#60A5FA' }}>zarezerwuj demo</Link>.</p>
      </section>
    </div>
  );
}
